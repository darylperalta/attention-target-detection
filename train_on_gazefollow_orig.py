...
def train():
    ...
    # Prepare data
    print("Loading Data")
    train_dataset = GazeFollowOrig(gazefollow_train_data, gazefollow_train_label,
    ...
    val_dataset = GazeFollowOrig(gazefollow_val_data, gazefollow_val_label,
    ...
    logdir = os.path.join(args.log_dir,
                          datetime.now().strftime('%Y-%m-%d_%H-%M-%S-orig'))
    ...
    for ep in range(args.epochs):
        for batch, (img, face, head_channel, gaze_heatmap, name, gaze_inside) in enumerate(train_loader):
            # print('batch', batch)
            # print('face', type(face), face.shape)
            # print('head', type(head_channel), head_channel.shape)
            # print('gaze_heatmap', type(gaze_heatmap), gaze_heatmap.shape)
            # print('name', type(name))
            # print('gaze_inside', type(gaze_inside), gaze_inside.shape)
